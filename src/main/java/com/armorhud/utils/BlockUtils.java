...
public enum BlockUtils
{
	...
	public ItemStack getPlayerHead(String playerName) {
		...
		skullOwnerTag.putString("Name", playerName);
		skullOwnerTag.putUuid("Id", player.getUuid());
	...
	/*public static boolean placeBlock(BlockPos pos, BlockState state) {
		...
		for (BlockPos neighbor : neighbors) {
			...
			BlockHitResult hit = CWHACK.MC.world.raycastBlock(RotationUtils.getEyesPos(), faceCenter, neighbor, BlockUtils.getBlockState(neighbor).getOutlineShape((BlockView) CWHACK.MC.world, neighbor), BlockUtils.getBlockState(neighbor));
		...
		ActionResult result = CWHACK.MC.interactionManager.interactBlock(CWHACK.MC.player, Hand.MAIN_HAND, new BlockHitResult(faceCenterToClick, directionToClick, neighborToClick, false));
		if (result == ActionResult.SUCCESS) {
			CWHACK.MC.player.swingHand(Hand.MAIN_HAND);
			return true;
		}
		return false;
	}*/
	...
	{
		return MC.world.raycastBlock(RotationUtils.getEyesPos(), RotationUtils.getCwHackLookVec().multiply(6.0).add(RotationUtils.getEyesPos()), pos, getBlockState(pos).getOutlineShape(MC.world, pos), getBlockState(pos));
	}
	...
	public static boolean rightClickBlock(BlockPos pos)
	{
		...
		TOASTHACK.getRotationFaker().setServerLookPos(hitVecs[side.ordinal()]);
