...
public class MathUtil {
    public static int compareDist(Entity entityA, Entity entityB) {
        return Float.compare(entityA.distanceTo(ClientInitializer.mc.player), entityB.distanceTo(ClientInitializer.mc.player));
    }
    ...
    public static boolean isInFOV(final Entity entity, double angle) {
        ...
        if(entity != null) {
            final double angleDiff = getAngleDifference(ClientInitializer.mc.player.getYaw(), getRotations(entity)[0]);
    ...
    public static float[] getRotationFromPosition(final double x, final double y, final double z) {
        final double xDiff = x - ClientInitializer.mc.player.getPos().getX();
        final double yDiff = y - (ClientInitializer.mc.player.getPos().getY() + ClientInitializer.mc.player.getEyeHeight(ClientInitializer.mc.player.getPose()));
        final double zDiff = z - ClientInitializer.mc.player.getPos().getZ();
    ...
    public static float[] getNeededRotations(final Vec3d vec) {
        final Vec3d eyesPos = ClientInitializer.mc.player.getEyePos();
    ...
    public static double getAngleToLookVec(final Vec3d vec) {
        ...
        final float currentYaw = MathHelper.wrapDegrees(ClientInitializer.mc.player.getYaw());
        final float currentPitch = MathHelper.wrapDegrees(ClientInitializer.mc.player.getPitch());
